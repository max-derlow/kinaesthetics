...
const bodies = new BodyStream ({
      posenet: posenet,
      architecture: modelArchitecture.ResNet50, //MobileNetV1, ResNet50
...
bodies.addEventListener('bodiesDetected', (e) => {
    ...
    //bodiess = e.detail.bodies.getNumOfBodies()
});
...
let twisterBodyParts = {
    ...
    leftEye: {drawn:false, colour: "green", xPos:0, yPos:0, string: "leftEye"},
    //rightElbow: {drawn:false, colour: "green", xPos:0, yPos:0, string: "rightElbow"},
    //leftElbow: {drawn:false, colour: "green", xPos:0, yPos:0, string: "leftElbow"},

    //rightAnkle:{drawn:false, colour: "indigo", xPos:0, yPos:0, string: "rightAnkle"},
};
...
function drawTwisterObject(connected, twisterObject){
    ...
    if(connected){
        ...
        colour = twisterBodyParts[bodyPart.part].colour;
    }

    let sideLength = twisterObjects[bodyPart.part].sideLength;
    let xBox = twisterObjects[bodyPart.part].x;
    let yBox = twisterObjects[bodyPart.part].y;
    if(xBox + 100 >= canvas.width){
        xBox = xBox - ((xBox + 100) - canvas.width);
        console.log("x gon fuck it to you")
    }
    if(yBox + 100 >= canvas.height){
        yBox = yBox - ((yBox + 100) - canvas.width);
        console.log("y gon fuck it to you")
    }
...
function draw(bodyPart, bodyPartObj, colour, radius){
    ...
    if(bodyPart.confidenceScore >= 0.25) {
        ctx.beginPath();
